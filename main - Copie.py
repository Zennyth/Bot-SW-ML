...
class Run:
	def __init__(self, path, imagePath, frameRateNonAnalysis, bounding_box, averageMouseDeplacement, averageWaitingTime, Refill, TypeName):
		...
		self._sct = mss()
		self._learn_inf = load_learner(path)
	...
	def checkTemplate(self, template, order):
		w, h = template['image'].shape[::-1]
		...
		result = cv2.matchTemplate(self._screen, template['image'], cv2.TM_SQDIFF)
		(minVal, maxVal, minLocation, maxLocation) = cv2.minMaxLoc(result)
		# print(minVal, template['name'])
	...
	def main(self):
		while self._start:
			if(self._i == fself._rameRateNonAnalysis):
				sct_img = sct.grab(self._bounding_box)
				output = np.array(Image.frombytes("RGB", sct_img.size, sct_img.bgra, "raw", "BGRX"))
				pred,pred_idx,probs = learn_inf.predict(output)
				print('nombre de run : ' + str(nbRun), end = '\r')
				# print(pred)
				...
				if verif(prev_preds):
					if not(self._endAction):
						if prev_preds[0] == 'Fail' or self._forcePath == 'Fail':
							if c < len(orderFail):
								checkTemplate(cv2.cvtColor(output,cv2.COLOR_RGB2GRAY), self._ui_template[self._orderFail[self._c][0]], self._orderFail)
								self._forcePathFail = 'Fail'
						if prev_preds[0] == 'Reward' or self._forcePath == 'Success':
							# for template in ui_template:
							# 	checkTemplate(cv2.cvtColor(output,cv2.COLOR_RGB2GRAY), ui_template[template])
							if self._c < len(self._orderSuccess):
								if self._c == 3:
									self._nbRun += 1
								checkTemplate(cv2.cvtColor(output,cv2.COLOR_RGB2GRAY), self._ui_template[self._orderSuccess[self._c][0]], self._orderSuccess)
...
Run(path, imagePath, frameRateNonAnalysis, bounding_box, averageMouseDeplacement, averageWaitingTime, Refill, TypeName)